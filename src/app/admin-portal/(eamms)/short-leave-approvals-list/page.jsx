...
const ShortLeaveApprovalList = () => {
  ...
  const fetchShortLeaves = async () => {
    try {
      const response = await axios.get("https://eamms.bluai.ai/api/get-short-leave-approvals-list", {
        headers: { Authorization: localStorage.getItem("token") },
      });
      setShortLeaves(response.data.shortLeaves || []);
    } catch (error) {
      console.error("Error fetching short leave approvals:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredData = useMemo(() => {
    ...
    return [...shortLeaves].reverse().filter((item) =>
      ...
      item?.cc_to?.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  // Helper function to format session text (add outside your component)
...
const columns = [
  ...
  { 
    ...
    cell: row => <div className="whitespace-pre-wrap break-words">{row.reason || "-"}</div>,
    width: "150px",
    wrap: true
  },
  { 
    ...
    cell: row => <div className="whitespace-pre-wrap break-words">{formatSessionText(row.session)}</div>,
    width: "150px", 
    center: "true" 
  },
  {
      ...
      <div className="w-[68vw] h-[68vh] max-w-7xl bg-white shadow-lg rounded-lg">
        ...
        <div className="p-6 overflow-auto">
          <div className="mb-4 relative">
            <input
              type="text"
              placeholder="Search short leave approvals..."
          ...
          <DataTable
            ...
            noDataComponent={
              <div className="py-8 text-center text-gray-500">
                No short leave approval records found
              </div>
            }
            fixedHeader
...
export default ShortLeaveApprovalList;