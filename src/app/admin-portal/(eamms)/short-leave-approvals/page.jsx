...
const ShortLeaveApprovals = () => {
  ...
  const fetchShortLeaves = async () => {
    try {
      setLoading(true);
      const response = await axios.get("https://eamms.bluai.ai/api/get-short-leave-approvals", {
        headers: {
  ...
  const updateLeaveStatus = async (id, action) => {
    try {
      const response = await axios.post(
        `https://eamms.bluai.ai/api/short-leave/confirm/${id}`,
        { action },
        {
          headers: {
            Authorization: localStorage.getItem("token"),
      ...
      if (response.data.status === "success") {
        setShortLeaves((prev) =>
          prev.map((item) =>
            item.id === id
              ? { ...item, leave_status1: response.data.leave_status1 }
              : item
          )
        );
      } else {
  ...
  const filteredData = useMemo(() => {
    if (!searchTerm) return shortLeaves.filter(leave => leave.leave_status1 !== "Withdrawn");
    
    return shortLeaves
      .filter(leave => leave.leave_status1 !== "Withdrawn")
      .filter(leave =>
        leave.user?.first_name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        leave.user?.username1?.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  { 
    ...
    cell: row => (
      <span className={`px-2 py-1 rounded-full text-xs font-medium ${
        row.leave_status1 === "Approved" ? "bg-green-100 text-green-800" :
        row.leave_status1 === "Rejected" ? "bg-red-100 text-red-800" :
      ...
      <div className="w-[68vw] h-[68vh] max-w-7xl bg-white shadow-xl rounded-lg">
        ...
        <div className="p-6 overflow-auto">
          <div className="mb-4 relative">
            <input
              type="text"
              placeholder="Search short leave applications..."
          ...
          <DataTable
            ...
            noDataComponent={
              <div className="py-8 text-center text-gray-500">
                No short leave applications found
              </div>
            }
            fixedHeader
...
export default ShortLeaveApprovals;