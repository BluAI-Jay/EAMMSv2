...
const RolesTable = () => {
  ...
  const fetchRoles = async () => {
    try {
      const response = await axios.get("https://eamms.bluai.ai/api/get-roles-data", {
        headers: {
  ...
  const handleDeleteRole = async (id) => {
    if (window.confirm("Are you sure you want to delete this role?")) {
      try {
        await axios.delete(`https://eamms.bluai.ai/api/roles-delete/${id}`, {
          headers: {
            Authorization: localStorage.getItem("token"),
          },
        });
  ...
  const handleAddRole = () => {
    navigation.push("/admin-portal/roles-create");
  };
    ...
    },
    {
      ...
      cell: (row) => (
        <div className="flex gap-2">
          <button
            onClick={() => navigation.push(`/admin-portal/roles-edit/${row.id}`)}
